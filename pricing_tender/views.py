...
from .serializers import PricingTenderSerializers, PricingTenderContractorSerializers ,OfferPriceSerializers#,PricingTinderCommentSerializers,
...
class PricingTenderViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        serialized_not_valid_users = [
            {'id': user.id, 'first_name': user.first_name, 'phone_number': str(user.phone_number),'role':str(user.role)}
            for user, value in not_valid_ids
        ]
        return Response([serializer.data,("not valid ids",serialized_not_valid_users)], status=status.HTTP_201_CREATED)
